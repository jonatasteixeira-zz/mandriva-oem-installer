...
class PhilcoDisk(WizardController):
...
class PhilcoInstaller(WizardController):
    def __init__(self, parent):
        WizardController.__init__(self, parent)
...
class PhilcoMaster(WizardController):
    def __init__(self, parent):
        WizardController.__init__(self, parent)
    ...
    def run(self):
        ...
        import subprocess
        if subprocess.call('grep -q partition /proc/cmdline', shell=True) != 0:
            self.create_part()

        self.interface_action("global_progress_bar", "message=Partições criadas!\nRestaurando partições\n")
